...
def setup_model_and_autoencoder(
    AutoEncoder, load: bool = True, tied: bool = False, bias: bool = False
):
    n_instances = 1
...
def test_autoencoder_init(AutoEncoder):
    ...
    assert (
        all_params_actual == all_params
    ), f"Incorrect parameters: expected {all_params}, got {all_params_actual}"

    autoencoder = setup_model_and_autoencoder(AutoEncoder, load=False, tied=True)
    all_params_actual = {name for name, param in autoencoder.named_parameters()}
    assert (
        all_params_actual == all_params_tied
    ), f"Incorrect parameters: expected {all_params_tied}, got {all_params_actual}"
...
def test_autoencoder_forward(AutoEncoder):
    ...
    for name, out, out_expected in zip(output_names, output, output_expected):
        t.testing.assert_close(
            out, out_expected, msg=f"autoencoder.forward() {name} is incorrect"
        )

    model, autoencoder, soln_autoencoder, h = setup_model_and_autoencoder(
        AutoEncoder, bias=True
    )
    t.testing.assert_close(
        autoencoder.W_enc,
        soln_autoencoder.W_enc,
...
def test_autoencoder_normalize_W_dec(AutoEncoder):
    print(
        "Testing with untied weights (normalize should change values of W_dec inplace)..."
    )
    model, autoencoder, soln_autoencoder, h = setup_model_and_autoencoder(
        AutoEncoder, tied=False
    )
    ...
    t.testing.assert_close(
        autoencoder.W_dec,
        soln_autoencoder.W_dec,
        msg="Test failed - did you forget to normalize the decoder weights inplace?",
    )

    print(
        "Testing with tied weights (normalize should return normalized & transposed W_enc, and not change W_enc)..."
    )
    model, autoencoder, soln_autoencoder, h = setup_model_and_autoencoder(
        AutoEncoder, tied=False
    )
...
def test_resample_neurons(resample_neurons):
    ...
    (
        t.testing.assert_close(
            new_b_enc[~features_are_dead], old_b_enc[~features_are_dead]
        ),
        "b_enc weights incorrectly changed where neurons are alive",
    )
