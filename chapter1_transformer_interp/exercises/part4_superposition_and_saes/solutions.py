...
from arena3.chapter1_transformer_interp.exercises.plotly_utils import hist
from utils import (
    plot_features_in_2d,
    plot_features_in_Nd,
...
RUN_CODE = {
    "TMS": True,  # run all non-essential code from sections 1-3
    "GEOM": False,  # run section 4 (there is no section 5)
    "SAE-TMS": True,  # run section 6
    "SAE-GELU": True,  # run section 7
}
...
if MAIN and RUN_CODE["TMS"]:
    ...
    assert (
        (corr0_is_active.float().mean(0) - feature_probability).abs().mean() < 0.002
    ...
    assert (
        (anticorr0_is_active.float().mean(0) - feature_probability).abs().mean() < 0.002
...
def resample_neurons(
    ...
    self.W_enc.data.transpose(-1, -2)[dead_features_mask] = (
...
def resample_neurons(
    ...
    for instance in range(self.cfg.n_instances):
        ...
        W_enc_norm_alive_mean = (
            ...
            else self.W_enc[instance, :, alive_features].norm(dim=0).mean().item()
        )
