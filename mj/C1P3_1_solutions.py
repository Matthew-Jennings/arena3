    ...
    if OVERRIDE_STEP_FRACTION > 0:
        ...
        with t.inference_mode():
            ...
            h = einops.einsum(
                batch,
                model.W,
                "batch inst feats, inst hidden feats -> inst hidden batch",
            )
...
if MAIN:
    ...
    utils.plot_features_in_2d(
        ...
        subplot_titles=[
            f"1 - S = 0.05 * {i:.2f}" for i in t.linspace(0, 1, cfg.n_inst).flip(0)
        ],
        n_rows=2,
    )
...
def compute_dimensionality(
    ...
    denominator = (
        einops.einsum(
            W_normalized,
            W,
            "inst d_hidden feats1, inst d_hidden feats2 -> inst feats1 feats2",
        )
        .pow(2)
        .sum(-1)
    )
        ...
        utils.animate_features_in_2d(
            {
                ...
                "h<sub>r</sub>": einops.rearrange(
                    h_r, "batch inst d_in -> inst d_in batch"
                ),
            },
...
if MAIN:
    ...
    cfg = Config(
        n_inst=n_inst, n_features=n_features, d_hidden=d_hidden, feat_mag_distn="jump"
    )
    model = Model(cfg=cfg, device=device)
    if OVERRIDE_STEP_FRACTION > 0:
        model.optimize(steps=int(OVERRIDE_STEP_FRACTION * 10_000))
        ...
        for architecture in ["standard", "gated"]:
            sae_cfg = SAEConfig(
                n_inst=n_inst, d_in=d_in, d_sae=d_sae, architecture=architecture
            )  # type: ignore
            ...
            utils.animate_features_in_2d(
                ...
                steps=data_log["steps"],
                filename=f"animation-training-resampling-{architecture}.html",
        ...
        rearrange_h = lambda h: einops.rearrange(h, "b i d_in -> i d_in b")  # noqa: E731
        utils.animate_features_in_2d(
            {
